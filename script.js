...
function renderIconItem(item) {
  ...
  return `
    <div class="group relative ${bgColor} rounded-xl p-3 cursor-pointer hover:shadow-md transition-all border border-transparent hover:border-gray-200 dark:hover:border-gray-600" onclick="handleItemClick('${item.id}', '${item.type}')">
      ...
      <button onclick="event.stopPropagation(); showActionMenu('${item.id}', '${item.type}')" class="absolute top-1 left-1 p-1.5 rounded-full bg-white dark:bg-gray-700 shadow opacity-70 hover:opacity-100 transition-opacity hover:bg-gray-100 dark:hover:bg-gray-600">
